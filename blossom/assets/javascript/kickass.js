...
(function(window) {
    ...
    var KickAss = new Class({
        ...
        isCampaign: function() {
            return window.KICKASSVERSION == "GAME_UK";
        }
    });
    ...
    var Menu = new Class({
        initialize: function(game) {
            ...
            this.url = GameGlobals.path('intermediate.html?url=' +
                encodeURIComponent(window.KICKASSURL || document.location.href) + "&preship=" + (window.KICKASSSHIPID || "") + "&is_campaign=" + (this.game.isCampaign() ? "true" : ""));
        },
    ...
    var MenuManager = new Class({
        ...
        create: function() {
            this.container = document.createElement('div');
            this.container.className = 'KICKASSELEMENT KICKASShidden';
            this.container.id = 'kickass-menu';
            this.container.style.bottom = '-250px';
            this.container.style.display = 'none';
            document.body.appendChild(this.container);
            this.container.innerHTML = '<div id="kickass-pointstab" class="KICKASSELEMENT">' + '<div id="kickass-bomb-menu" class="KICKASSELEMENT KICKASShidden">' + '<ul id="kickass-bomb-list" class="KICKASSELEMENT">' + '</ul>' + '</div>' + '<div id="kickass-weapons-menu" class="KICKASSELEMENT KICKASShidden" style="display:none">' + '<ul id="kickass-weapons-list" class="KICKASSELEMENT">' + '</ul>' + '</div>' + '<div id="kickass-pointstab-wrapper" class="KICKASSELEMENT">' + '<div id="kickass-points" class="KICKASSELEMENT">' +
                this.numPoints + '</div>' + '<div id="kickass-esctoquit" class="KICKASSELEMENT">Press esc to quit</div>' +
                this.getShareHTML() + '</div>' + '<ul id="kickass-pointstab-menu" class="KICKASSELEMENT">' + '<li class="KICKASSELEMENT"><a class="KICKASSELEMENT" id="kickass-link-highscores" href="#">Submit score</a></li>' + '<li class="KICKASSELEMENT"><a class="KICKASSELEMENT" id="kickass-link-menu" href="#">Menu</a></li>' + '<li class="last-li KICKASSELEMENT"><a class="KICKASSELEMENT" id="kickass-link-ships" href="#">Switch ship</a></li>' + '</ul>' + '</div>';
        ...
        getShareHTML: function() {
            if (this.game.isCampaign()) {
                var url = 'http%3A%2F%2Fwww.facebook.com%2FGAMEstore'
                ...
                return '<iframe class="KICKASSELEMENT kickass-like" src="//www.facebook.com/plugins/like.php?href=http%3A%2F%2Fwww.facebook.com%2Fkickassapp&amp;send=false&amp;layout=button_count&amp;width=47&amp;show_faces=false&amp;action=like&amp;colorscheme=dark&amp;font=arial&amp;height=21" scrolling="no" frameborder="0" style="border:none; overflow:hidden; width:80px; height:21px;" allowTransparency="true"></iframe>';
            }
        },
        onGameReady: function() {
        ...
        addPoints: function(killed, pos) {
            ...
            GameGlobals.stats.increasePointsGainedWithPoints(points);
            GameGlobals.kickass.ui.addPointsBubbleAt(pos, points);
        },
    ...
    var initKickAss = function() {
        if (window.KICKASSVERSION == 'GAME_UK') {
            var script = document.createElement("script");
            script.src = "//hi.kickassapp.com/game-uk.js";
            //script.src = "http://dev.kickassapp.com/static/game-uk.js";
