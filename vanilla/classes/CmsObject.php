...
class CmsObject implements ArrayAccess
{
    ...
    /**
     * @var Delphinium\Vanilla\Classes\PluginVector A reference to the Plugin containing the object.
     */
    protected $plugin;
     ...
     */
    public function __construct(PluginVector $plugin = null)
    {
        $this->plugin = $plugin;
    }
     ...
     * @return mixed Returns a CMS object instance or null if the object wasn't found.
     */
    public static function load($plugin, $fileName, $fullPath)
    {
        if (!FileHelper::validatePath($fileName, static::getMaxAllowedPathNesting())) {
            throw new ApplicationException(Lang::get('cms::lang.cms_object.invalid_file', ['name'=>$fileName]));
        }
...
var_dump($this->fileName);
     ...
     * @param \Cms\Classes\Theme $plugin Specifies a theme the file belongs to.
