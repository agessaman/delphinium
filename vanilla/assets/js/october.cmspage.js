...
+function ($) { "use strict";
    ...
    CmsPage.prototype.updateTemplateList = function(type) {
        var $form = $('#vanilla-side-panel form[data-template-type='+type+']'),
    ...
    CmsPage.prototype.registerHandlers = function() {
        ...
        $document.on('open.oc.list', '#vanilla-side-panel', this.proxy(this.onOpenDocument))
        ...
        $document.on('click', '#vanilla-side-panel form button[data-control=create-template], #vanilla-side-panel form li a[data-control=create-template]', this.proxy(this.onCreateTemplateClick))
        $document.on('click', '#vanilla-side-panel form button[data-control=delete-template]', this.proxy(this.onDeleteTemplateClick))
        ...
        $document.on('click', '#vanilla-master-tabs > div.tab-content > .tab-pane.active .control-componentlist a.remove', this.proxy(this.onComponentRemove))
    ...
    CmsPage.prototype.onOpenDocument = function(event) {
         ...
         */
         if ($('#vanilla-master-tabs').data('oc.tab').goTo(tabId))
            return false
        ...
        $form.request('onOpenTemplate', {
            ...
            $('#vanilla-master-tabs').ocTab('addTab', data.tabTitle, data.tab, tabId, $form.data('type-icon'))
        }).always(function() {
            $.oc.stripeLoadIndicator.hide()
    ...
    CmsPage.prototype.onTabClosed = function(ev) {
        ...
        if ($('> div.tab-content > div.tab-pane', '#vanilla-master-tabs').length == 0)
            this.setPageTitle('')
    }
    ...
    CmsPage.prototype.onTabShown = function(ev) {
        ...
        if ($target.closest('[data-control=tab]').attr('id') != 'vanilla-master-tabs')
            return

        var dataId = $target.closest('li').attr('data-tab-id'),
            title = $target.attr('title'),
            $sidePanel = $('#vanilla-side-panel')
    ...
    CmsPage.prototype.onInitTab = function(ev, data) {
        ...
        if ($(ev.target).attr('id') != 'vanilla-master-tabs')
            return
    ...
    CmsPage.prototype.onAfterAllTabsClosed = function(ev) {
        var $sidePanel = $('#vanilla-side-panel')
    ...
    CmsPage.prototype.onAjaxUpdate = function(ev) {
        var dataId = $('#vanilla-master-tabs .nav-tabs li.active').attr('data-tab-id'),
            $sidePanel = $('#vanilla-side-panel')
    ...
    CmsPage.prototype.onAjaxSuccess = function(ev, context, data) {
        ...
        if (data.tabTitle !== undefined) {
            $('#vanilla-master-tabs').ocTab('updateTitle', $(element).closest('.tab-pane'), data.tabTitle)
        ...
        $('#vanilla-master-tabs').ocTab('updateIdentifier', $(element).closest('.tab-pane'), tabId)
       ...
       var  $form = $(ev.target).closest('[data-template-type]'),
        ...
        $form.request('onCreateTemplate', {
           data: {type: type}
            ...
            $('#vanilla-master-tabs').ocTab('addTab', data.tabTitle, data.tab, tabId, $form.data('type-icon') + ' new-template')
    ...
    CmsPage.prototype.onInspectorHiding = function(ev, values) {
        var element = ev.target,
            ...
            $componentList = $('#vanilla-master-tabs > div.tab-content > .tab-pane.active .control-componentlist .layout'),
    ...
    CmsPage.prototype.onComponentClick = function(ev) {
        ...
        var $componentList = $('#vanilla-master-tabs > div.tab-content > .tab-pane.active .control-componentlist .layout')
    ...
    CmsPage.prototype.updateModifiedCounter = function() {
        ...
        $('> div.tab-content > div.tab-pane[data-modified]', '#vanilla-master-tabs').each(function(){
    ...
    CmsPage.prototype.reloadForm = function(form) {
        var 
            ...
            tabId = data.type + '-' + data.theme + '-' + data.path,
            tabs = $('#vanilla-master-tabs').data('oc.tab'),
            tab = tabs.findByIdentifier(tabId),
            self = this
        ...
        $form.request('onOpenTemplate', {
            ...
            $('#vanilla-master-tabs').ocTab('updateTab', tab, data.tabTitle, data.tab)
            $('#vanilla-master-tabs').ocTab('unmodifyTab', tab)
            self.updateModifiedCounter()
        }).always(function() {
            $.oc.stripeLoadIndicator.hide()
        }).fail(function(jqXHR, textStatus, errorThrown) {
    ...
    CmsPage.prototype.updateLayouts = function(form) {
        $(form).request('onGetTemplateList', {
            success: function(data) {
                $('#vanilla-master-tabs > .tab-content select[name="settings[layout]"]').each(function(){
                    var 
                        $select = $(this),
                        value = $select.val()
